...
dyad_independent_ergm_list_full_network <- lapply(bipartite_graph_full_network_sna_list, function(graph) {
  ergm(graph ~ edges +
         b2cov("age") +
         b2factor("gender") +
...
for (i in seq_along(dyad_independent_ergm_list_full_network)) {
  ...
  print(summary(dyad_independent_ergm_list_full_network[[i]]))
